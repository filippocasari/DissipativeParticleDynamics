...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        particle_list[i]=Particle(x, y, vx, vy, m, L, xcell, ycell, i , test);
    ...
    cmap_walls.insert(make_pair("color", "red"));
    ...
    while (iter < 20000) {
                ...
                for (int j = -1; j < 2; j++) {
                    ...
                    for (auto &part2: cell_list[cell_x_new][cell_y_new]) {
                        ...
                        if(r>0.0 ){
                            ...
                            if(d_v_x!=0.0 || d_v_y!=0.0){
                                F_x_D = -gamma * w_d * (d_x_hat)* d_v_x* d_x_hat;
                                F_y_D = -gamma * w_d * (d_y_hat)* d_v_y* d_y_hat;
                            }
                            if (xi_map.find({part.id, part2.id}) == xi_map.end()) {
                                ...
                                //return 3;
                            } else if((xi_map.find({part2.id, part.id}) == xi_map.end())){
                                // found
                            ...
                            F_tot_x+= (F_x_C*dt) + (F_x_D*dt) + (F_x_R*(sqrt(dt)));
                            F_tot_y+= F_y_C*dt + F_y_D*dt + F_y_R*(sqrt(dt));
                        }
                        }
        ...
        int counter_wall =0;
        int counter_fluid =0;
        for (int i = 0; i < N_walls+N_fluid; i++) {
